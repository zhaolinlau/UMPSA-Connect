...
export default defineEventHandler(async (event) => {
	...
	if (query.post_id) {
		...
		const { data, error } = await client.from('bookmarks').select('*, announcements(*), events(*), posts(*, comments(*), votes(*), bookmarks(*))').eq('user_id', query.user_id).order('created_at', { ascending: false })
